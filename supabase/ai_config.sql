...
CREATE TABLE IF NOT EXISTS ai_config (
    ...
    model_id VARCHAR(100) NOT NULL,       -- AI 模型标识符（如 'deepseek-chat', 'gemini-pro'）
    api_key TEXT NOT NULL,                -- 对用的 API 密钥
...
-- 4. 表注释
COMMENT ON TABLE ai_config IS 'AI 配置表：存储用户自定义的 AI 模型参数和密钥';
COMMENT ON COLUMN ai_config.model_id IS 'AI 模型唯一标识，如 deepseek-chat 或 custom';
