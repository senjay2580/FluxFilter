...
const DevCommunity: React.FC = () => {
  ...
  const fetchGitHub = useCallback(async (loadMore = false) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://api.github.com/search/repositories?q=created:>${since}&sort=stars&order=desc&per_page=15&page=${currentPage}`
      );
      ...
      setHasMore(prev => ({ ...prev, github: repos.length === 15 }));
    } catch (err) {
  ...
  const fetchStackOverflow = useCallback(async (loadMore = false) => {
    ...
    try {
      const res = await fetch(
        `https://api.stackexchange.com/2.3/questions?order=desc&sort=hot&site=stackoverflow&pagesize=15&page=${currentPage}&filter=withbody`
      );
      ...
      setHasMore(prev => ({ ...prev, stackoverflow: data.has_more }));
    } catch (err) {
  ...
  // 进入页面时自动加载 GitHub 和 Stack Overflow
  useEffect(() => {
    if (githubRepos.length === 0 && !loading.github) {
      fetchGitHub();
    }
    if (soQuestions.length === 0 && !loading.stackoverflow) {
      fetchStackOverflow();
    }
  ...
  return (
    <div className="min-h-full">
        ...
        {tabs.map(tab => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${
              activeTab === tab.id && !isExternalTab(tab.id)
      ...
      {/* 同步按钮 - 仅 GitHub 和 Stack Overflow */}
      {!isIframeTab(activeTab) && (
        <button
          onClick={handleRefresh}
          disabled={currentLoading}
          className="w-full mb-4 py-3 bg-gradient-to-r from-cyber-lime/20 to-emerald-500/20 hover:from-cyber-lime/30 hover:to-emerald-500/30 border border-cyber-lime/30 rounded-xl text-cyber-lime font-medium transition-all disabled:opacity-50 flex items-center justify-center gap-2"
        >
          {currentLoading ? (
            <>
